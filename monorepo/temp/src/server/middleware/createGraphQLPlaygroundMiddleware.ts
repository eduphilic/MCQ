...
import { Context as ApolloContext, UserRole } from "../../api";
...
export function createGraphQLPlaygroundMiddleware(
  ...
  return async (ctx: Context, next: () => Promise<any>) => {
    if (ctx.url === "/graphql" && ctx.method === "GET") {
      ...
      if (apolloContext.userService.getCurrentUserRole() !== UserRole.ADMIN) {
        ctx.throw(400, "Access Denied");
        return;
      }

      // Render GraphQL Playground page with CSRF token injected.
      // Page source from here:
