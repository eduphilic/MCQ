...
/**
 * Renders the GraphQL Playground which is normally handled as part of the
 * middlewares applied by Apollo Server. This controller renders the page with
 * an attached CSRF token.
 *
 * https://github.com/prisma/graphql-playground#as-html-page
 */
@Controller("graphql")
export class GraphQLController {
  @Get()
  @Header("X-Robots-Tag", "noindex")
  @HttpCode(process.env.NODE_ENV === "production" ? 404 : 200)
  playground(@CsrfToken() csrfToken: string) {
    if (process.env.NODE_ENV !== "development") {
      return undefined;
    }
    return generatePlaygroundPageContents(csrfToken);
  }
}

/**
 * Returns the HTML for the GraphQL Playground configured to use the supplied
 * CSRF token.
 *
 * https://github.com/prisma/graphql-playground/blob/master/packages/graphql-playground-react/src/localDevIndex.tsx#L47
 */
function generatePlaygroundPageContents(csrfToken: string) {
  ...
  <body>
    ...
    <script type="text/javascript">
      window.addEventListener('load', function (event) {
        ...
        GraphQLPlayground.init(root, ${JSON.stringify(playgroundConfig)});
      })
    </script>
  </body>
  </html>
