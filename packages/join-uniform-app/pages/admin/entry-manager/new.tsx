...
type Props = EntryManagerGetEntriesQuery;
...
export default function AdminEntryManagerNewEntryPageContainer() {
  return (
    <EntryManagerGetEntriesComponent>
      {({ data, loading, error }) =>
        loading || error || !data ? (
          ...
          <AdminEntryManagerNewEntryPage entries={data.entries} />
        )
      }
    </EntryManagerGetEntriesComponent>
  );
...
function AdminEntryManagerNewEntryPage(props: Props) {
  const { entries } = props;
